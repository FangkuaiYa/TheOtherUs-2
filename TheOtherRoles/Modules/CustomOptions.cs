...
namespace TheOtherRoles {
    public class CustomOption {
        ...
        {
            return _HeaderBackground ??= Helpers.loadSpriteFromResources("TheOtherRoles.Resources.HeaderBackground.png", 100f);
        }
        public static Sprite GetDividerSprite()
        {
            return _HeaderDivider ??= Helpers.loadSpriteFromResources("TheOtherRoles.Resources.HeaderDivider.png", 100f);
        }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse(TheOtherRolesPlugin.VersionString)) {
                    ...
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "Host Info: Pasting vanilla settings failed, TOU Options applied!");
                } else {
    ...
    class LobbyViewSettingsPatch{
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) {
            ...
            foreach (var option in relevantOptions) {
                ...
                if ((int)optionType == 99) {
                    viewSettingsInfoPanel.titleText.outlineWidth = -1f;
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options) {
            ...
            foreach (CustomOption option in options) {
                if (option.isHeader) {
                    ...
                    categoryHeaderMasked.Title.text = option.heading != "" ? option.heading : option.name;
                    categoryHeaderMasked.Title.outlineWidth = -1f;
